...
def app_metrics_fetch(metric_type:str, load_type:str, end_date:str ,demo_flag:int=1):
    assert len(load_type) == 1
    assert demo_flag in [0, 1]
    if demo_flag == 1:
        print("Running Demo")
    else:
        print("Running paid API Request")
    ...
    try:
        for app_name, devices in music_apps.items(): 
            ...
            print(f"Getting reponses for the following app: {app_name}") 
            for country in country_list:
                ...
                for device, code in devices[0].items():
                    ...
                    # Use hardcoded values if demo to save money on app request
                    if demo_flag == 0:
                        api_response = app_metric.request(code, beginning_date, country, device)
                    else:
                    ...
                    count += 1
                    print(f"API response for country code: {country}, written to file")
                    
                    #break the loop after one iteration for demo purposes
                    if demo_flag == 1:
                        break
                if demo_flag == 1:
                    break
...
def main():
    ...
    app_metrics_fetch(metric_type, load_type, end_date, 1)
    return True
